...
export async function exportDaySalesReportText(
  ...
  const totalRevenue = daySales.reduce((sum, sale) => sum + sale.total, 0);
  const parts = [
    `ADVANCE COAT — Reporte del ${dayLabel}`,
    `Ventas: ${daySales.length}`,
    `Recaudación: $${totalRevenue.toFixed(2)}`,
    '============================',
    ...daySales.map((sale) => buildSaleTicketText(sale)),
  ];
  const text = parts.join('\n\n');
  const fileName = `reporte-${format(day, 'yyyy-MM-dd')}.txt`;
